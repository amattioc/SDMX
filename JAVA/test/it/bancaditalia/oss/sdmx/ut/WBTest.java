...
public class WBTest {
	...
	public void testGetDSDIdentifier() throws SdmxException {
		DSDIdentifier keyF = SdmxClientHandler.getDSDIdentifier("WB", "WDI");
	...
	public void testGetFlows() throws SdmxException {
		Map<String, String> f = SdmxClientHandler.getFlows("WB", "WDI");
	...
	public void testGetDimensions() throws SdmxException {
		List<Dimension> dim = SdmxClientHandler.getDimensions("WB", "WDI");
	...
	public void testGetCodes() throws SdmxException {
			Map<String, String> codes = SdmxClientHandler.getCodes("WB", "WDI", "REF_AREA");
	...
	public void testGetTimeSeriesFromID() throws SdmxException {
		List<PortableTimeSeries> res = SdmxClientHandler.getTimeSeries("WB", "WDI.A.SP_POP_TOTL.USA", "2000", "2010");
		assertNotNull("Null time series result", res);
		String annual = res.get(0).getName();
		assertEquals("Wrong name for first time series", "WDI.A.SP_POP_TOTL.USA", annual);
		String start = res.get(0).getTimeSlots().get(0);
		assertEquals("Wrong start date for time series", "2000", start);
		res = SdmxClientHandler.getTimeSeries("WB", "WDI.A.SP_POP_TOTL.", "2000", "2010");
		assertNotNull("Null time series result", res);
		assertEquals("Wrong time series result", res.size() > 0, true);
		res = SdmxClientHandler.getTimeSeries("WB", "WDI.A.SP_POP_TOTL.ITA+USA", null, null);
		assertNotNull("Null time series result", res);
		assertEquals("Wrong time series result", res.size() > 0, true);
	}
